...
「Notionに〜を作りたい」「〜を管理したい」だけの依頼は、プロパティや使う場面が
本人の中でもまだ固まっていないことが多い。次のいずれかに該当する場合だけ、
DDLを組み立てる前にこのステップを挟む:

- 依頼に具体的なプロパティ・DB名・既存DBとの関係が一切含まれていない
  （例:「タスクを管理したい」「用語をまとめたい」レベル）
- 何のために・いつ使うのかが読み取れない

該当しない場合(既に項目や置き場所まで具体的な依頼)は Step 0 を飛ばして
作成の原則に進む。

**質問だけで止めない。依頼の言葉から想像できる具体案を2〜3個先に出し、
「近いのはどれか」「これで足りない点は」を聞く。** ゼロから
「どんな項目が欲しいですか」と丸投げしない(homepage-companionと同じ、
初心者を専門用語で止めない方針)。案を出すときは:

1. **用途を1つ具体的に言語化する。** 「いつ・どんな場面で開くか」が
   決まるとプロパティの中身が決まる(例: 「ゼミ前に見返す」→理解度で
   絞り込みたい、が用語集DBの「要復習」ビューにつながった)
2. **近い既存DB・スキル定義があれば真っ先に示す。** `skills-` リポジトリの
   各スキルが `references/notion_db_schema.md` を持っていたり、ワークスペース
   に類似DBが既にある場合は「これに近い形でどうか」とたたき台にする
   （ゼロから提案するよりイメージが伝わりやすい）
3. 案を1つに決め打ちしない。**2〜3案(プロパティの粒度・置き場所などの
   バリエーション)を並べて選んでもらう**か、「まずこれで作って、使いながら
   足す」という最小構成案を必ず選択肢に含める
4. イメージが固まったら以降の作成の原則・DDLテンプレートに進む。
   固まる前にDDLを組み立てない
