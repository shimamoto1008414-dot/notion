...
両方を同時に直す。以下の2つは実際に作成済み(🎓大学院ページ配下にサブページを
1枚挟んで設置。DDLはそのときに使った定義を記録として残す)。
...
設置場所: 🎓大学院 →「タスク」ページ配下（サブページ）。仕事/大学院/私用を横断する内容だが、実際に使っているページが大学院だけなのでここに設置（カテゴリで横断管理する）
```sql
CREATE TABLE (
  "タスク名" TITLE,
...
```
完了タスクは削除・アーカイブせず、ビューのフィルタで隠すだけにする（スキル側の運用ルール）。ビュー: 全プロパティ(テーブル) / 残タスク(状況≠完了) / 今日(緊急度=至急×状況≠完了) / 隙間時間(所要時間=短×状況≠完了) / 待ち(状況=待ち) / すべて(追加日降順)。
